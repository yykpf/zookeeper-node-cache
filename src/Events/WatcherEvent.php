...
class WatcherEvent {
    ...
    public function run()
    {
        $this->zookeeper = new \Zookeeper($this->host, [$this, 'watch']);
        // 设置报错级别
        ...
        switch ($eventType) {
            ...
            case \Zookeeper::SESSION_EVENT:
                ...
                if (3 == $connectionState) {
                    $res = $this->getAndCacheChild($this->rootPath);
                    if (!empty($res)) {
                        $this->cacheStrategy->setCacheConf($res);
                    }
                }
     ...
     * @param $root
     */
    protected function getAndCacheChild($rootPath): array
    {
        $res = [];
        if ($this->zookeeper->exists($rootPath)) {
            CommonFunctions::commonOutput("watch child: $rootPath");
            $nodes = $this->zookeeper->getChildren($rootPath, [$this, 'watch']);
            if (empty($nodes)) {
                $res = $this->getAndCacheNode($rootPath);
            } else {
                foreach ($nodes as $node) {
                    CommonFunctions::commonOutput("cache node: $node");
                    $res = array_merge($res, $this->getAndCacheChild($rootPath . '/' . $node));
                }
            }
        }
